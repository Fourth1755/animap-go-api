-- id = uuid v7
INSERT INTO studios(id, name, image, created_at, updated_at)
	VALUES 
    (
        '019566f2-46cb-78bb-aeea-56bf7bca0248', 
        'A-1 Pictures', 
        'https://cdn.myanimelist.net/s/common/company_logos/4713c58b-833f-4c92-bf4a-0e2f7af8a461_600x600_i?s=925a453653da58d385adb82b5d423a69', 
        now(), 
        now()
    ),
    (  
        '019566f3-1284-71bc-89dc-b57bad2b953c',
        'Madhouse', 
        'https://cdn.myanimelist.net/s/common/company_logos/e68488ab-f0a0-411f-850a-18fb3e21b96c_600x600_i?s=21618c9c3183ffded748d303a253b637', 
        now(), 
        now()
    ),
    (
        '019566f3-b1c7-79cf-ab96-9380233b049a',
        'Bones', 
        'https://cdn.myanimelist.net/s/common/company_logos/969047f0-a8ec-475e-ad0d-6e0d5cd8e17f_600x600_i?s=4145bdb95a29f3fe1447baa8045a7420', 
        now(), 
        now()
    ),
    (
        '019566f4-23c6-7ece-8d98-291c73878d61',
        'White Fox', 
        'https://cdn.myanimelist.net/s/common/company_logos/40ba3310-9602-47fc-beb2-cc346adada09_600x600_i?s=6e3600896f107186d1a8dad0a3222d43', 
        now(), 
        now()
    ),
    (
        '019566f4-75f0-7900-934c-a865fd25ef2e',
        'Toei Animation', 
        'https://cdn.myanimelist.net/s/common/company_logos/33d49515-685a-4133-8ad3-41b09197e88d_600x600_i?s=cd6405cb06051286ce2bfbd4ce645443', 
        now(), 
        now()
    ),
    (
        '019566f4-da4e-7cdd-97c5-e8251e7949ec',
        'Wit Studio', 
        'https://cdn.myanimelist.net/s/common/company_logos/e7e64f9e-23f6-4c74-9813-cb4fcdb600cf_600x600_i?s=37f6a1b3342db61d87d4e50803fd6fd6', 
        now(), 
        now()
    ),
    (
        '019566f5-28c8-7c0f-86c1-0ded38288b43',
        'Bandai Namco Pictures', 
        'https://cdn.myanimelist.net/s/common/company_logos/0cae085b-c75f-443c-a997-976b354f8cae_600x600_i?s=127539e086823a0c362a8bd7194ae5de', 
        now(), 
        now()
    ),
    (
        '019566f5-7e38-7103-8f16-e48541fc608c',
        'Sunrise', 
        'https://cdn.myanimelist.net/s/common/company_logos/6a9279a7-1aeb-4a2b-bceb-4e7f24fae7b1_600x600_i?s=3550689bfb655805cd6d4d3b3987cc91', 
        now(), 
        now()
    ),
    (
        '019566f5-c93a-7629-8477-5856397c1518',
        'Pierrot', 
        'https://cdn.myanimelist.net/s/common/company_logos/a0524dfa-5a6d-40a7-8a1e-233e3822acb5_600x600_i?s=48123c150ab3365033f471461a50acc6', 
        now(), 
        now()
    ),
    (
        '019566f6-152d-7949-834a-c457cba6ab6d',
        'TMS Entertainment', 
        'https://cdn.myanimelist.net/s/common/company_logos/be6fb172-8033-4f5c-a904-4ad4de90dbdd_600x600_i?s=9cf1899ebb8c743af6c1c54678909b9b', 
        now(), 
        now()
    ),
    (
        '019566f6-9851-772b-b1ed-d6e7399e45a9',
        'Kyoto Animation', 
        'https://cdn.myanimelist.net/s/common/company_logos/b066ff17-81d3-40db-b1f2-2927de70c0e3_600x600_i?s=edb149cf051e2d7984975063a1b3b3a7', 
        now(), 
        now()
    ),
    (
        '019566f6-ecbc-7ce9-84ea-d88be0ed5b4a',
        'Shaft', 
        'https://cdn.myanimelist.net/s/common/company_logos/6abfb420-5815-4a62-b978-cbbf9b868fa0_600x600_i?s=5fe7fdaf8e4e09c14c58d7ac6fc29f80', 
        now(), 
        now()
    ),
    (
        '019566f7-30c7-7867-95d9-735ff970d860',
        'MAPPA', 
        'https://cdn.myanimelist.net/s/common/company_logos/e3a5163d-3b09-4e98-922b-79180a75539f_600x600_i?s=3289c478fd611569ebccd7ff076151df', 
        now(), 
        now()
    ),
    (
        '019566f7-c32b-7b99-9f9c-b100c7cb4a17',
        'CoMix Wave Films', 
        'https://cdn.myanimelist.net/s/common/company_logos/7a3b5971-939c-456e-8d62-2e09bc8a414c_600x600_i?s=887af1b5bacebab80acdf6c62e73e591', 
        now(), 
        now()
    ),
    (
        '019566f8-2104-7a8a-8d41-309587224001',
        'Studio Signpost', 
        'https://cdn.myanimelist.net/s/common/company_logos/4a8f5425-ad12-4d54-bb08-6122f820736a_600x600_i?s=366c367a1b6534a592e84f65dd7393ad', 
        now(), 
        now()
    ),
    (
        '019566f8-5788-7d91-a182-f0ff65fad6c5',
        'CloverWorks', 
        'https://cdn.myanimelist.net/s/common/company_logos/75875b81-17bb-4f7e-a06f-bb149d54687e_600x600_i?s=b90a570d03511f70dbac7e04869f4835', 
        now(), 
        now()
    ),
    (
        '019566f8-ad3d-799a-b54a-925a911e9d04',
        'Trigger', 
        'https://cdn.myanimelist.net/s/common/company_logos/bc3f892a-8581-45b1-8a95-81b6ac518f3d_600x600_i?s=d2a2e7f05478dbb999b17d35c44445db', 
        now(), 
        now()
    ),
    (
        '019566fa-1918-7819-b40d-840ffab8874a',
        'DandeLion Animation Studio', 
        'https://cdn.myanimelist.net/s/common/company_logos/c6f1b337-3c33-445d-a84b-0147ca2675bc_600x600_i?s=b61036f704d307bf81ea8c267291fafa', 
        now(), 
        now()
    ),
    (
        '019566fa-dc59-7258-bc3f-f3d9f0f6c182',
        'ufotable', 
        'https://cdn.myanimelist.net/s/common/company_logos/03171393-4a85-451d-a025-4a3f05d1aede_600x600_i?s=48ebfd25c277dd148d41f88568f60aa6', 
        now(), 
        now()
    ),
    (
        '019566fc-3a78-7ee6-ba3c-7d8258dc829c',
        'B.CMAY PICTURES', 
        'https://cdn.myanimelist.net/s/common/company_logos/090899e7-7649-41cb-9d67-a22b97ae4c9c_600x600_i?s=118777258b8d040d46d14d55f55f6457', 
        now(), 
        now()
    ),
    (
        '019566fd-0842-7206-b22f-2b9b89ea46b4',
        'Studio Ghibli', 
        'https://cdn.myanimelist.net/s/common/company_logos/e6d02dfe-71e9-49d2-bef1-68e585c2605e_600x600_i?s=f8bba4a0f7ae97f80c95e463c7529bd6', 
        now(), 
        now()
    ),
    (
        '019566fe-2840-79b6-acfb-76ab46da019b',
        'Kinema Citrus', 
        'https://cdn.myanimelist.net/s/common/company_logos/720b6054-9637-436d-851b-da7a1740828e_600x600_i?s=75114ead680445183b369acd185b7307', 
        now(), 
        now()
    ),
    (
        '019566ff-0a91-7944-8733-07f9b56d9880',
        'LAN Studio', 
        'https://cdn.myanimelist.net/s/common/company_logos/328de67f-1438-4542-842a-3a5a52fa1ac7_600x600_i?s=13ab6809d11cb0bcec69fca166fe66fc', 
        now(), 
        now()
    ),
    (
        '0195670c-355c-7c01-88a8-11c68157fec6',
        'Gainax', 
        'https://cdn.myanimelist.net/s/common/company_logos/7bba2118-127e-47a3-855f-0b284689f3da_600x600_i?s=c62c0bef4c82c81b750746955a423f64', 
        now(), 
        now()
    ),
    (
        '0195fc54-668f-723a-a993-0e6c5048a098',
        'Doga Kobo', 
        'https://cdn.myanimelist.net/s/common/company_logos/e820af66-29b6-48c2-bfba-43e9bd5d8c27_600x600_i?s=367d70746c45aaa5c1bb7295aeb46820', 
        now(), 
        now()
    ),
    (
        '0195670e-53d4-79f1-9183-941c06099b6f',
        'Studio Chizu', 
        'https://cdn.myanimelist.net/s/common/company_logos/8caa8a79-47ce-4857-bb7e-9065c7e4fdbb_600x600_i?s=3722063712d129991c148938c4c78458', 
        now(), 
        now()
    ),
    (
        '0195670f-4d10-75ac-a3fc-e4eeccc891ba',
        'Science SARU', 
        'https://cdn.myanimelist.net/s/common/company_logos/4c810cc0-ae5b-4b4d-98e7-926c719275b9_600x600_i?s=9ab5260497621fcca4a578fed011a1c3', 
        now(), 
        now()
    ),
    (
        '01956710-5fbf-7ae0-97c6-d73d1c7d14a9',
        'J.C.Staff', 
        'https://cdn.myanimelist.net/s/common/company_logos/076ec06c-a090-41b5-971e-2fc2ae446f5e_600x600_i?s=5872ea7d2c75469d2d296574a5c8c1fb', 
        now(), 
        now()
    ),
    (
        '0195671f-fb66-788f-9456-b573e7d44c22',
        'Studio Bind', 
        'https://cdn.myanimelist.net/s/common/company_logos/17557685-c55a-4aa3-9990-a13667e7c1b5_600x600_i?s=c81759de42fc570ba43e173dff286257', 
        now(), 
        now()
    ),
    (
        '01968746-54d4-7178-a788-40ecb937824c',
        'SILVER LINK.', 
        'https://cdn.myanimelist.net/s/common/company_logos/1a9d42cc-7f3d-4a45-8d4d-137d2594e797_600x600_i?s=6d48b120428087576743810225726392', 
        now(), 
        now()
    ),
    (
        '01968747-e6cc-7806-a264-9f445e4e7b49',
        'OLM', 
        'https://cdn.myanimelist.net/s/common/company_logos/4b41f888-3ec1-478e-aadd-882b78b4e3af_600x600_i?s=f2cd42bd452b467ee5deed362ca179d6', 
        now(), 
        now()
    ),
    (
        '0196874d-cdfc-73f5-a383-7deec5f88cca',
        'LIDENFILMS', 
        'https://cdn.myanimelist.net/s/common/company_logos/c7537634-ea2e-4e5b-acb3-53b4fadc8d51_600x600_i?s=80fedafe87a731b52dd7c32f18195f2b', 
        now(), 
        now()
    );
    	